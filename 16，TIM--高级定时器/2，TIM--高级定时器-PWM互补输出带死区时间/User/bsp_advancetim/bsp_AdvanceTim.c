...
static void ADVANCE_TIM_GPIO_Config(void) 
{
  GPIO_InitTypeDef GPIO_InitStructure;
	...
	RCC_APB2PeriphClockCmd(ADVANCE_TIM_CH1_GPIO_CLK, ENABLE);				// 输出比较通道 GPIO 初始化
  GPIO_InitStructure.GPIO_Pin =  ADVANCE_TIM_CH1_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(ADVANCE_TIM_CH1_PORT, &GPIO_InitStructure);
  
	RCC_APB2PeriphClockCmd(ADVANCE_TIM_CH1N_GPIO_CLK, ENABLE);			// 输出比较通道互补通道 GPIO 初始化
  GPIO_InitStructure.GPIO_Pin =  ADVANCE_TIM_CH1N_PIN;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(ADVANCE_TIM_CH1N_PORT, &GPIO_InitStructure);
  
	RCC_APB2PeriphClockCmd(ADVANCE_TIM_BKIN_GPIO_CLK, ENABLE);			// 输出比较通道刹车通道 GPIO 初始化
...
/*--------------------时基结构体初始化-------------------------*/
	...
	/*-------------------刹车和死区结构体初始化-------------------*/	
	TIM_BDTRInitTypeDef TIM_BDTRInitStructure;											// 有关刹车和死区结构体的成员具体可参考BDTR寄存器的描述
  TIM_BDTRInitStructure.TIM_OSSRState = TIM_OSSRState_Enable;
  TIM_BDTRInitStructure.TIM_OSSIState = TIM_OSSIState_Enable;
  TIM_BDTRInitStructure.TIM_LOCKLevel = TIM_LOCKLevel_1;
  TIM_BDTRInitStructure.TIM_DeadTime = 11;												// 输出比较信号死区时间配置，此处配置为152ns
  TIM_BDTRInitStructure.TIM_Break = TIM_Break_Enable;	
  TIM_BDTRInitStructure.TIM_BreakPolarity = TIM_BreakPolarity_High;// 当BKIN引脚检测到高电平的时候，输出比较信号被禁止，就好像是刹车一样
