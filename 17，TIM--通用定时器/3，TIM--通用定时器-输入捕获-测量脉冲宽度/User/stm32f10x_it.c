...
void GENERAL_TIM_INT_FUN(void)
{
	...
	{	
		if ( TIM_ICUserValueStructure.Capture_StartFlag == 0 )											// 第一次捕获
		{			
			...
			GENERAL_TIM_OCxPolarityConfig_FUN(GENERAL_TIM, TIM_ICPolarity_Falling);		// 当第一次捕获到上升沿之后，就把捕获边沿配置为下降沿     		
			TIM_ICUserValueStructure.Capture_StartFlag = 1;														// 开始捕获标准置1			
		}
		// 下降沿捕获中断
		else 																																				// 第二次捕获
		{			
			TIM_ICUserValueStructure.Capture_CcrValue = 
			GENERAL_TIM_GetCapturex_FUN (GENERAL_TIM);											// 获取捕获比较寄存器的值，这个值就是捕获到的高电平的时间的值
