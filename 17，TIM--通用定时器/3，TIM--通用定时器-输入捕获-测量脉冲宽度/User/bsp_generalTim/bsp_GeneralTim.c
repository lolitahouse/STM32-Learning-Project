...
/*--------------------时基结构体初始化-------------------------*/	
  TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;	
	...
	TIM_TimeBaseStructure.TIM_Prescaler= GENERAL_TIM_PSC;			// 驱动CNT计数器的时钟 = Fck_int/(psc+1)	
	...
	TIM_ICInitStructure.TIM_Channel = GENERAL_TIM_CHANNEL_x;						// 配置输入捕获的通道，需要根据具体的GPIO来配置	
	TIM_ICInitStructure.TIM_ICPolarity = GENERAL_TIM_STRAT_ICPolarity;	// 输入捕获信号的极性配置	
	...
	TIM_ICInit(GENERAL_TIM, &TIM_ICInitStructure);											// 定时器输入捕获初始化	
	
  TIM_ClearFlag(GENERAL_TIM, TIM_FLAG_Update|GENERAL_TIM_IT_CCx);							// 清除更新和捕获中断标志位   
	TIM_ITConfig (GENERAL_TIM, TIM_IT_Update | GENERAL_TIM_IT_CCx, ENABLE ); 		// 开启更新和捕获中断		
